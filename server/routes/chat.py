...
async def tp2_build(model_id: str, request: Request):
    """TP=2 engine build: each rank builds its own shard locally.
    Tactics are tuned to local hardware — cross-machine engine transfer
    fails to deserialize on heterogeneous GPUs."""
    r0, r1 = await _get_tp_nodes(request)
    if not r0 or not r1:
        return JSONResponse(status_code=400, content={"error": "Need 2 GPUs online"})
    log.info(f"TP2 build: rank0={r0} and rank1={r1} each build own shard")

    res0, res1 = await asyncio.gather(
        _relay_to_daemon(
            {"type": "build", "model_id": model_id, "tp": 2, "rank": 0},
            timeout_s=60.0, node_id=r0,
        ),
        _relay_to_daemon(
            {"type": "build", "model_id": model_id, "tp": 2, "rank": 1},
            timeout_s=60.0, node_id=r1,
        ),
    )
    return {"rank0": res0, "rank1": res1}
